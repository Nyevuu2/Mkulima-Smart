...
export default function Weather({ county, crop = "Maize" }) {
  ...
  useEffect(() => {
    const fetchLiveClimateAnalytics = async () => {
      try {
        ...
        // 1. Fetch data directly from Open-Meteo API
        const openMeteoUrl = `https://api.open-meteo.com/v1/forecast?latitude=${coords.lat}&longitude=${coords.lon}&daily=temperature_2m_max,precipitation_sum&timezone=Africa/Nairobi`;
        ...
        if (omResponse.ok) {
          ...
          const transformedWeeklyRows = omData.daily.time.map((timeStr, index) => {
            const dateObj = new Date(timeStr);
            return {
              day: daysOfWeek[dateObj.getDay()],
              temp: Math.round(omData.daily.temperature_2m_max[index]),
              rain: Math.round(omData.daily.precipitation_sum[index])
            };
          });
          setWeeklyForecast(transformedWeeklyRows);
        }

        // 2. Query your updated backend API link destination
        const advisoryResponse = await fetch(`http://localhost:8000/api/weather-advice?county=${encodeURIComponent(activeCounty)}&crop=${encodeURIComponent(activeCrop)}`);
        if (advisoryResponse.ok) {
          const advData = await advisoryResponse.json();
          setAdvisoryPayload(advData);
        }
      } catch (err) {
        console.error("Failed to map dynamic weather streaming connections:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [activeCounty, activeCrop]); // 🚨 Re-fires immediately when either activeCounty or activeCrop changes!
